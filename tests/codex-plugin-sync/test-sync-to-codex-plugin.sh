...
EOF
    ...
    cat > "$repo/assets/superskills-small.svg" <<'EOF'
...
write_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/superskills/skills/example"
    printf 'fixture keep\n' > "$repo/plugins/superskills/.fixture-keep"
    cat > "$repo/plugins/superskills/skills/example/SKILL.md" <<'EOF'
# Example Skill

Fixture content.
EOF
    git -C "$repo" add plugins/superskills/.fixture-keep
    git -C "$repo" add plugins/superskills/skills/example/SKILL.md
...
add_openai_agent_metadata_fixture() {
    ...
    mkdir -p "$repo/plugins/superskills/skills/example/agents"

    cat > "$repo/plugins/superskills/skills/example/agents/openai.yaml" <<'EOF'
interface:
  display_name: "Example"
  short_description: "Destination-owned OpenAI metadata"
EOF

    git -C "$repo" add plugins/superskills/skills/example/agents/openai.yaml
...
write_synced_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/superskills/.codex-plugin" \
        "$repo/plugins/superskills/.private-journal" \
        "$repo/plugins/superskills/assets" \
        "$repo/plugins/superskills/skills/example/agents" \
        "$repo/plugins/superskills/skills/example"

    cat > "$repo/plugins/superskills/.codex-plugin/plugin.json" <<EOF
{
  "name": "superskills",
  "version": "$MANIFEST_VERSION"
}
EOF
    ...
    cat > "$repo/plugins/superskills/assets/superskills-small.svg" <<'EOF'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1 1"></svg>
EOF

    printf 'png fixture\n' > "$repo/plugins/superskills/assets/app-icon.png"
...
EOF
    ...
    printf 'tracked keep\n' > "$repo/plugins/superskills/.private-journal/keep.txt"

    git -C "$repo" add \
        plugins/superskills/.codex-plugin/plugin.json \
        plugins/superskills/assets/app-icon.png \
        plugins/superskills/assets/superskills-small.svg \
        plugins/superskills/skills/example/agents/openai.yaml \
        plugins/superskills/skills/example/SKILL.md \
        plugins/superskills/.private-journal/keep.txt
...
write_stale_ignored_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/superskills/.private-journal"
    printf 'fixture keep\n' > "$repo/plugins/superskills/.fixture-keep"
    printf 'stale ignored leak\n' > "$repo/plugins/superskills/.private-journal/leak.txt"
    git -C "$repo" add plugins/superskills/.fixture-keep
...
main() {
    ...
    dirty_skill_path="$dirty_apply_dest/plugins/superskills/skills/example/SKILL.md"
    noop_openai_metadata_path="$noop_apply_dest/plugins/superskills/skills/example/agents/openai.yaml"
    ...
    assert_contains "$preview_section" "assets/superskills-small.svg" "Preview includes SVG asset"
    ...
    assert_not_contains "$preview_output" "Assets (superskills-small.svg, app-icon.png) will be seeded from" "Preview omits assets seeding note"
    ...
    assert_branch_absent "$dest" "sync/superskills-*" "Preview does not create sync branch in destination checkout"
    ...
    assert_contains "$bootstrap_output" "Mode:     BOOTSTRAP (creating plugins/superskills/ when absent)" "Bootstrap preview describes directory creation"
    ...
    assert_path_absent "$bootstrap_dest/plugins/superskills" "Bootstrap preview does not create destination plugin directory"
    ...
    assert_branch_absent "$bootstrap_dest" "bootstrap/superskills-*" "Bootstrap preview does not create bootstrap branch in destination checkout"

    echo ""
    echo "Apply assertions..."
    assert_equals "$dirty_apply_status" "1" "Dirty local apply exits with failure"
    assert_contains "$dirty_apply_output" "ERROR: local checkout has uncommitted changes under 'plugins/superskills'" "Dirty local apply reports protected destination path"
    ...
    assert_branch_absent "$dirty_apply_dest" "sync/superskills-*" "Dirty local apply does not create sync branch in destination checkout"
...
Locally modified fixture content." "Dirty local apply preserves tracked working-tree file content"
    ...
    assert_branch_absent "$noop_apply_dest" "sync/superskills-*" "Clean no-op local apply does not create sync branch in destination checkout"
