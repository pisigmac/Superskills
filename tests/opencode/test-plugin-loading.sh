#!/usr/bin/env bash
# Test: Plugin Loading
# Verifies that the superskills plugin loads correctly in OpenCode
set -euo pipefail
...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/superskills.js"
...
skill_count=$(find "$SUPERSKILLS_SKILLS_DIR" -name "SKILL.md" | wc -l)
if [ "$skill_count" -gt 0 ]; then
    echo "  [PASS] Found $skill_count skills"
else
    echo "  [FAIL] No skills found in $SUPERSKILLS_SKILLS_DIR"
    exit 1
fi

# Test 3: Check using-superskills skill exists (critical for bootstrap)
echo "Test 3: Checking using-superskills skill (required for bootstrap)..."
if [ -f "$SUPERSKILLS_SKILLS_DIR/using-superskills/SKILL.md" ]; then
    echo "  [PASS] using-superskills skill exists"
else
    echo "  [FAIL] using-superskills skill not found (required for bootstrap)"
    exit 1
fi

# Test 4: Verify plugin JavaScript syntax (basic check)
echo "Test 4: Checking plugin JavaScript syntax..."
if node --check "$SUPERSKILLS_PLUGIN_FILE" 2>/dev/null; then
...
if grep -q 'configDir}/skills/superskills/' "$SUPERSKILLS_PLUGIN_FILE"; then
