...
# 1. Create in superskills location (lowest priority)
mkdir -p "$SUPERSKILLS_SKILLS_DIR/priority-test"
cat > "$SUPERSKILLS_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Superskills version of priority test skill
---
# Priority Test Skill (Superskills Version)

This is the SUPERSKILLS version of the priority test skill.

PRIORITY_MARKER_SUPERSKILLS_VERSION
EOF
...
if [ -f "$SUPERSKILLS_SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] Superskills version exists"
else
    echo "  [FAIL] Superskills version missing"
    exit 1
fi
...
# Test 2: Document personal vs bundled superskills priority
echo ""
echo "Test 2: Documenting personal vs superskills priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled superskills version instead of personal version"

# Test 3: Document project vs bundled superskills priority
echo ""
echo "Test 3: Documenting project vs personal/superskills priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled superskills version instead of project version"

# Test 4: Test a non-colliding bundled superskills skill is still available
echo ""
echo "Test 4: Testing non-colliding superskills skill remains available..."

mkdir -p "$SUPERSKILLS_SKILLS_DIR/superskills-only-test"
cat > "$SUPERSKILLS_SKILLS_DIR/superskills-only-test/SKILL.md" <<'EOF'
---
name: superskills-only-test
description: Superskills-only priority test skill
---
# Superskills Only Test Skill

PRIORITY_MARKER_SUPERSKILLS_ONLY_VERSION
EOF

run_opencode output "$TEST_HOME/test-project" "Call the skill tool with name \"superskills-only-test\". Show the exact content including any PRIORITY_MARKER text."
assert_contains "$output" "PRIORITY_MARKER_SUPERSKILLS_ONLY_VERSION" "Non-colliding superskills skill is still registered"
