...
run_present_file_check() {
    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$SUPERSKILLS_PLUGIN_FILE" present
}

run_missing_file_check() {
    mv "$SUPERSKILLS_SKILLS_DIR/using-superskills/SKILL.md" "$TEST_HOME/using-superskills.SKILL.md.bak"

    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$SUPERSKILLS_PLUGIN_FILE" missing
}
