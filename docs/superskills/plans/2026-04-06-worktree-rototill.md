...
> **For agentic workers:** REQUIRED SUB-SKILL: Use superskills:subagent-driven-development (recommended) or superskills:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make superskills defer to native harness worktree systems when available, fall back to manual git worktrees when not, and fix three known finishing bugs.
...
**Spec:** `docs/superskills/specs/2026-04-06-worktree-rototill-design.md`
...
2. **Check for existing global directory:**
   ...
   ls -d ~/.config/superskills/worktrees/$project 2>/dev/null
   ```
...
Global directories (`~/.config/superskills/worktrees/`) need no verification.
...
# For global: path="~/.config/superskills/worktrees/$project/$BRANCH_NAME"
...
**If worktree path is under `.worktrees/` or `~/.config/superskills/worktrees/`:** Superskills created this worktree — we own cleanup.
...
- **Fix:** Only clean up worktrees under `.worktrees/` or `~/.config/superskills/worktrees/`
...
```markdown
- **superskills:using-git-worktrees** - REQUIRED: Set up isolated workspace before starting
```

to:

```markdown
- **superskills:using-git-worktrees** - Ensures isolated workspace (creates one or verifies existing)
```
