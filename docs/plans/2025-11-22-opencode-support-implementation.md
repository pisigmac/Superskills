...
> **For agentic workers:** REQUIRED SUB-SKILL: Use superskills:executing-plans to implement this plan task-by-task.

**Goal:** Add full superskills support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/superskills-codex` (lines 40-74)
...
- Reference: `.codex/superskills-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'superskills' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/superskills-codex` (lines 212-280)
 ...
 * (personal skills override superskills skills).
 *
 * @param {string} skillName - Name like "superskills:brainstorming" or "my-skill"
 * @param {string} superskillsDir - Path to superskills skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, superskillsDir, personalDir) {
    // Strip superskills: prefix if present
    const forceSuperskills = skillName.startsWith('superskills:');
    const actualSkillName = forceSuperskills ? skillName.replace(/^superskills:/, '') : skillName;

    // Try personal skills first (unless explicitly superskills:)
    if (!forceSuperskills && personalDir) {
    ...
    // Try superskills skills
    if (superskillsDir) {
        const superskillsPath = path.join(superskillsDir, actualSkillName);
        const superskillsSkillFile = path.join(superskillsPath, 'SKILL.md');
        if (fs.existsSync(superskillsSkillFile)) {
            return {
                skillFile: superskillsSkillFile,
                sourceType: 'superskills',
                skillPath: actualSkillName
            };
        }
    }
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/superskills-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/superskills-codex` (add import at top)
...
Run: `node -c .codex/superskills-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/superskills-codex
...
**Files:**
- Modify: `.codex/superskills-codex` (lines 40-74)
...
Run: `.codex/superskills-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/superskills-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/superskills-codex` (lines 16-38, approximately)
...
Run: `.codex/superskills-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/superskills.js`
...
/**
 * Superskills plugin for OpenCode.ai
 *
...
const superskillsSkillsDir = path.join(homeDir, '.config/opencode/superskills/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const SuperskillsPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/superskills.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/superskills.js
...
**Files:**
- Modify: `.opencode/plugin/superskills.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "superskills:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > superskills)
          const resolved = skillsCore.resolveSkillPath(
            skill_name,
            superskillsSkillsDir,
            personalSkillsDir
          );
...
Run: `node -c .opencode/plugin/superskills.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the superskills and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const superskillsSkills = skillsCore.findSkillsInDir(
            superskillsSkillsDir,
            'superskills',
            3
          );
          ...
          const allSkills = [...personalSkills, ...superskillsSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install superskills skills to ~/.config/opencode/superskills/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'superskills:';
...
```javascript
    'session.started': async () => {
      // Read using-superskills skill content
      const usingSuperskillsPath = skillsCore.resolveSkillPath(
        'using-superskills',
        superskillsSkillsDir,
        personalSkillsDir
      );

      let usingSuperskillsContent = '';
      if (usingSuperskillsPath) {
        const fullContent = fs.readFileSync(usingSuperskillsPath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingSuperskillsContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- Superskills skills: \`superskills:skill-name\` (from ~/.config/opencode/superskills/skills/)
...
`;
      ...
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/superskills')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/superskills && git pull` to update superskills.' :
        '';

      // Return context to inject into session
      return {
        context: `<EXTREMELY_IMPORTANT>
You have superskills.

**Below is the full content of your 'superskills:using-superskills' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**

${usingSuperskillsContent}
...
```markdown
# Installing Superskills for OpenCode
...
### 1. Install Superskills Skills

```bash
# Clone superskills skills to OpenCode config directory
mkdir -p ~/.config/opencode/superskills
git clone https://github.com/pisigmac/Superskills.git ~/.config/opencode/superskills
```

### 2. Install the Plugin

The plugin is included in the superskills repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/superskills/.opencode/plugin/superskills.js`
...
ln -s ~/.config/opencode/superskills/.opencode/plugin/superskills.js .opencode/plugin/superskills.js
```
...
```
use use_skill tool with skill_name: "superskills:brainstorming"
```
...
Personal skills override superskills skills with the same name.

## Updating

```bash
cd ~/.config/opencode/superskills
git pull
```
...
1. Check plugin file exists: `ls ~/.config/opencode/superskills/.opencode/plugin/superskills.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/superskills/skills`
...
- Report issues: https://github.com/pisigmac/Superskills/issues
- Documentation: https://github.com/pisigmac/Superskills
```
...
Superskills works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
Run: `.codex/superskills-codex use-skill superskills:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/superskills-codex bootstrap | head -30`
...
ls -l .opencode/plugin/superskills.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/superskills.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/superskills-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/superskills-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/superskills.js` created with tools and hooks
