...
Add full superskills support for OpenCode.ai using a native OpenCode plugin architecture that shares core functionality with the existing Codex implementation.

## Background

OpenCode.ai is a coding agent similar to Claude Code and Codex. Previous attempts to port superskills to OpenCode (PR #93, PR #116) used file-copying approaches. This design takes a different approach: building a native OpenCode plugin using their JavaScript/TypeScript plugin system while sharing code with the Codex implementation.
...
2. **Platform-Specific Wrappers**
   - Codex: CLI script (`.codex/superskills-codex`)
   - OpenCode: Plugin module (`.opencode/plugin/superskills.js`)

3. **Skill Directories**
   - Core: `~/.config/opencode/superskills/skills/` (or installed location)
   - Personal: `~/.config/opencode/skills/` (shadows core skills)

### Code Reuse Strategy

Extract common functionality from `.codex/superskills-codex` into shared module:
...
```javascript
{
  ...
  schema: z.object({
    skill_name: z.string().describe('Name of skill (e.g., "superskills:brainstorming")')
  }),
...
1. **Inject using-superskills content**
   - Full content of the using-superskills skill
...
```javascript
// .opencode/plugin/superskills.js
const skillsCore = require('../../lib/skills-core');
const path = require('path');
const fs = require('fs');
const { z } = require('zod');

export const SuperskillsPlugin = async ({ client, directory, $ }) => {
  const superskillsDir = path.join(process.env.HOME, '.config/opencode/superskills');
  const personalDir = path.join(process.env.HOME, '.config/opencode/skills');

  return {
    'session.started': async () => {
      const usingSuperskills = await readSkill('using-superskills');
      const skillsList = await findAllSkills();
      const toolMapping = getToolMappingInstructions();

      return {
        context: `${usingSuperskills}\n\n${skillsList}\n\n${toolMapping}`
      };
    },
...
├── .codex/
│   ├── superskills-codex        # UPDATED: Use skills-core
│   ├── superskills-bootstrap.md
│   └── INSTALL.md
├── .opencode/
│   ├── plugin/
│   │   └── superskills.js       # NEW: OpenCode plugin
...
1. Create `lib/skills-core.js`
   - Extract frontmatter parsing from `.codex/superskills-codex`
...
2. Update `.codex/superskills-codex` to use shared core
...
1. Create `.opencode/plugin/superskills.js`
