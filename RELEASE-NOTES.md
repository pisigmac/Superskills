# Superskills Release Notes

## v5.1.0 (2026-04-30)

### Removals

- **Legacy slash commands removed** — `/brainstorm`, `/execute-plan`, and `/write-plan` are gone. They were deprecated stubs that did nothing but tell the user to invoke the corresponding skill. Invoke `superskills:brainstorming`, `superskills:executing-plans`, and `superskills:writing-plans` directly instead. (#1188)
- **`superskills:code-reviewer` named agent removed** — the agent was the plugin's only named agent and was used by exactly two skills, while every other reviewer/implementer subagent in the repo dispatches `general-purpose` with a prompt template alongside its skill. The agent's persona and checklist have been merged into `skills/requesting-code-review/code-reviewer.md` as a self-contained Task-dispatch template. Anyone dispatching `Task (superskills:code-reviewer)` should switch to `Task (general-purpose)` with the prompt template instead. (PR #1299)
...
- **Provenance-based cleanup** — `finishing-a-development-branch` only cleans up worktrees inside `.worktrees/` (created by superskills); anything outside is left alone. Fixes #940 (Option 2 was incorrectly cleaning up worktrees), #999 (merge-then-remove ordering), and #238 (`cd` to repo root before `git worktree remove`).
...
- **New harness PRs require a session transcript** — most past new-harness integrations copied skill files or wrapped with `npx skills` instead of loading the `using-superskills` bootstrap at session start. The acceptance test ("Let's make a react todo list" must auto-trigger `brainstorming` in a clean session) and a complete transcript are now required.

### Codex Plugin Mirror Tooling

New `sync-to-codex-plugin` script mirrors superskills into the OpenAI Codex plugin marketplace as `prime-radiant-inc/openai-codex-plugins`. Path/user-agnostic so any team member can run it. (PR #1165)
...
- **SessionStart context injection** — Copilot CLI v1.0.11 added support for `additionalContext` in sessionStart hook output. The session-start hook now detects the `COPILOT_CLI` environment variable and emits the SDK-standard `{ "additionalContext": "..." }` format, giving Copilot CLI users the full superskills bootstrap at session start. (Original fix by @culinablaz in PR #910)
- **Tool mapping** — added `references/copilot-tools.md` with the full Claude Code to Copilot CLI tool equivalence table
- **Skill and README updates** — added Copilot CLI to the `using-superskills` skill's platform instructions and README installation section

### OpenCode Fixes

- **Skills path consistency** — the bootstrap text no longer advertises a misleading `configDir/skills/superskills/` path that didn't match the runtime path. The agent should use the native `skill` tool, not navigate to files by path. Tests now use consistent paths derived from a single source of truth. (#847, #916)
...
- **Added `package.json`** so OpenCode can install superskills as an npm package from git.
...
- `GEMINI.md` @imports `using-superskills` skill and tool mapping table at session start
- Gemini CLI tool mapping reference (`skills/using-superskills/references/gemini-tools.md`) — translates Claude Code tool names (Read, Write, Edit, Bash, etc.) to Gemini CLI equivalents (read_file, write_file, replace, etc.)
...
- Specs (brainstorming output) now save to `docs/superskills/specs/YYYY-MM-DD-<topic>-design.md`
- Plans (writing-plans output) now save to `docs/superskills/plans/YYYY-MM-DD-<feature-name>.md`
...
Writing-plans no longer offers a choice between subagent-driven and executing-plans. On harnesses with subagent support (Claude Code, Codex), subagent-driven-development is required. Executing-plans is reserved for harnesses without subagent capability, and now tells the user that Superskills works better on a subagent-capable platform.
...
- `lib/brainstorm-server/` — WebSocket server with browser helper library, session management scripts, and dark/light themed frame template ("Superskills Brainstorming" with GitHub link)
...
- Design spec and implementation plan in `docs/superskills/`
...
Added explicit priority ordering to using-superskills:

1. User's explicit instructions (CLAUDE.md, AGENTS.md, direct requests) — highest priority
2. Superskills skills — override default system behavior
...
Added `<SUBAGENT-STOP>` block to using-superskills. Subagents dispatched for specific tasks now skip the skill instead of activating the 1% rule and invoking full skill workflows.
...
This fix should dramatically improve superskills skills compliance and should reduce the chances of Claude entering its native plan mode unintentionally.
...
**Using-superskills workflow graph intercepts EnterPlanMode**
...
Changed `async: true` to `async: false` in hooks.json. When async, the hook could fail to complete before the model's first turn, meaning using-superskills instructions weren't in context for the first message.
...
The `superskills-codex` bootstrap CLI, Windows `.cmd` wrapper, and related bootstrap content file have been removed. Codex now uses native skill discovery via `~/.agents/skills/superskills/` symlink, so the old `use_skill`/`find_skills` CLI tools are no longer needed.
...
The synchronous SessionStart hook blocked the TUI from entering raw mode on Windows, freezing all keyboard input. Running the hook async prevents the freeze while still injecting superskills context.
...
- Windows doesn't respect shebangs, so directly invoking the extensionless `superskills-codex` script triggered an "Open with" dialog. All invocations now prefixed with `node`.
...
Superskills for OpenCode now uses OpenCode's native `skill` tool instead of custom `use_skill`/`find_skills` tools. This is a cleaner integration that works with OpenCode's built-in skill discovery.

**Migration required:** Skills must be symlinked to `~/.config/opencode/skills/superskills/` (see updated installation docs).
...
**Strengthened using-superskills skill for explicit skill requests**
...
The underlying skills (`superskills:brainstorming`, `superskills:executing-plans`, `superskills:writing-plans`) remain available for Claude to invoke autonomously. This change prevents confusion when Claude would invoke a command that just redirects to a skill anyway.
...
Fixed a confusing pattern where Claude would invoke a skill via the Skill tool, then try to Read the skill file separately. The `using-superskills` skill now explicitly states that the Skill tool loads skill content directly—no need to read files.

- Added "How to Access Skills" section to `using-superskills`
- Changed "read the skill" → "invoke the skill" in instructions
- Updated slash commands to use fully qualified skill names (e.g., `superskills:brainstorming`)
...
**Skill priority in using-superskills**
...
- **Rationalizations table** in using-superskills - Scannable format including new entries: "I need more context first", "Let me explore first", "This feels productive"
...
- **OpenCode Bootstrap Refactor**: Switched from `chat.message` hook to `session.created` event for bootstrap injection
  ...
  - Explicitly tells the model that using-superskills is already loaded to prevent redundant skill loading
...
- **OpenCode Support**: Native JavaScript plugin for OpenCode.ai
  ...
  - Three-tier skill priority: project > personal > superskills
...
- Optimized superskills bootstrap to eliminate redundant skill execution. The `using-superskills` skill content is now provided directly in session context, with clear guidance to use the Skill tool only for other skills. This reduces overhead and prevents the confusing loop where agents would execute `using-superskills` manually despite already having the content from session start.
...
- Added unified `superskills-codex` script with bootstrap/use-skill/find-skills commands
- Cross-platform Node.js implementation (works on Windows, macOS, Linux)
- Namespaced skills: `superskills:skill-name` for superskills skills, `skill-name` for personal
- Personal skills override superskills skills when names match
...
- Updated terminology: "Superskills skills" instead of "Core skills"

### Files Added
- `.codex/INSTALL.md` - Installation guide for Codex users
- `.codex/superskills-bootstrap.md` - Bootstrap instructions with Codex adaptations
- `.codex/superskills-codex` - Unified Node.js executable with all functionality

**Note:** Codex support is experimental. The integration provides core superskills functionality but may require refinement based on user feedback.

## v3.2.3 (2025-10-23)

### Improvements

**Updated using-superskills skill to use Skill tool instead of Read tool**
...
### Files Changed
- Updated: `skills/using-superskills/SKILL.md` - Changed tool references from Read to Skill

## v3.2.2 (2025-10-21)

### Improvements

**Strengthened using-superskills skill against agent rationalization**
...
### Files Changed
- Updated: `skills/using-superskills/SKILL.md` - Added three layers of enforcement to prevent skill-skipping rationalization
...
- Added `superskills:code-reviewer` agent to plugin's `agents/` directory
- Agent provides systematic code review against plans and coding standards
- Previously required users to have personal agent configuration
- All skill references updated to use namespaced `superskills:code-reviewer`
- Fixes #55

### Files Changed
- New: `agents/code-reviewer.md` - Agent definition with review checklist and output format
- Updated: `skills/requesting-code-review/SKILL.md` - References to `superskills:code-reviewer`
- Updated: `skills/subagent-driven-development/SKILL.md` - References to `superskills:code-reviewer`
...
- All internal skill references now use `superskills:` namespace prefix
- Updated format: `superskills:test-driven-development` (previously just `test-driven-development`)
...
- **Fixed command syntax in README** (#44) - Updated all command references to use correct namespaced syntax (`/superskills:brainstorm` instead of `/brainstorm`). Plugin-provided commands are automatically namespaced by Claude Code to avoid conflicts between plugins.
...
# Superskills v2.0.0 Release Notes

## Overview

Superskills v2.0 makes skills more accessible, maintainable, and community-driven through a major architectural shift.

The headline change is **skills repository separation**: all skills, scripts, and documentation have moved from the plugin into a dedicated repository ([pisigmac/Superskills-skills](https://github.com/pisigmac/Superskills-skills)). This transforms superskills from a monolithic plugin into a lightweight shim that manages a local clone of the skills repository. Skills auto-update on session start. Users fork and contribute improvements via standard git workflows. The skills library versions independently from the plugin.
...
**The biggest change:** Skills no longer live in the plugin. They've been moved to a separate repository at [pisigmac/Superskills-skills](https://github.com/pisigmac/Superskills-skills).

**What this means for you:**

- **First install:** Plugin automatically clones skills to `~/.config/superskills/skills/`
...
1. Your old `~/.config/superskills/.git` will be backed up to `~/.config/superskills/.git.bak`
2. Old skills will be backed up to `~/.config/superskills/skills.bak`
3. Fresh clone of pisigmac/Superskills-skills will be created at `~/.config/superskills/skills/`

### Removed Features

- **Personal superskills overlay system** - Replaced with git branch workflow
- **setup-personal-superskills hook** - Replaced by initialize-skills.sh
...
- Clones pisigmac/Superskills-skills on first run
...
- `SUPERSKILLS_SKILLS_ROOT` set to `~/.config/superskills/skills`
...
- Removed obsolete setup-personal-superskills call from session-start
...
- Prominent link to superskills-skills repo
...
**Removed:**
- `skills/` directory (82 files) - Now in pisigmac/Superskills-skills
- `scripts/` directory - Now in pisigmac/Superskills-skills/skills/using-skills/
- `hooks/setup-personal-superskills.sh` - Obsolete

**Modified:**
- `hooks/session-start.sh` - Use skills from ~/.config/superskills/skills
- `commands/brainstorm.md` - Updated paths to SUPERSKILLS_SKILLS_ROOT
- `commands/write-plan.md` - Updated paths to SUPERSKILLS_SKILLS_ROOT
- `commands/execute-plan.md` - Updated paths to SUPERSKILLS_SKILLS_ROOT
...
- PR #2: Personal superskills overlay system (later replaced)
...
```bash
# In Claude Code
/plugin marketplace add pisigmac/Superskills-marketplace
/plugin install superskills@superskills-marketplace
```
...
1. **Backup your personal skills** (if you have any):
   ```bash
   cp -r ~/.config/superskills/skills ~/superskills-skills-backup
   ```

2. **Update the plugin:**
   ```bash
   /plugin update superskills
   ```
...
- Skills repository is now at https://github.com/pisigmac/Superskills-skills
...
**Full Changelog:** https://github.com/pisigmac/Superskills/compare/dd013f6...main
**Skills Repository:** https://github.com/pisigmac/Superskills-skills
